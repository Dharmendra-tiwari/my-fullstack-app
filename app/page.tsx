...
export default function Home() {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center p-24 bg-gray-50">
      ...
      {/* Add Task Form */}
      <form onSubmit={handleAddTask} className="w-full max-w-3xl mb-12 bg-white p-6 rounded-xl shadow-md">
            ...
            {tasks.map((task) => (
              <li
                ...
                className={`p-6 rounded-xl shadow-md border-l-6 transition-all hover:scale-102 ${
                  task.completed ? "border-green-500 bg-green-50/80" : "border-amber-500 bg-amber-50/80"
                }`}
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className={`px-4 py-2 rounded-full text-sm font-semibold ${
                    task.completed ? "bg-green-200 text-green-800" : "bg-amber-200 text-amber-800"
                  }`}>
      ...
      <div className="mt-16">
        <Link href="/about" className="px-10 py-5 bg-purple-600 text-white text-lg font-semibold rounded-xl hover:bg-purple-700 transition shadow-lg">
          Next Steps →
        </Link>
      </div>
    </main>
